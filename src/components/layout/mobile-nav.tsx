...
import { Menu, LayoutDashboard, Ticket, ParkingSquare, Users, MapPin, Settings, BarChart3, LogOut } from "lucide-react";
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="h-8 w-8 -ml-1 lg:hidden">
      ...
      >
        <div className="px-5 pt-6 pb-2 space-y-3">
          <Image src="/logo-white.webp" alt="SmartLots" width={120} height={30} />
        </div>
        ...
        <ScrollArea className="flex-1 py-3 px-3">
          <nav className="flex flex-col gap-5">
            {navGroups.map((group) => (
              <div key={group.label}>
                <p className="text-overline text-muted-foreground mb-2 px-2">{group.label}</p>
